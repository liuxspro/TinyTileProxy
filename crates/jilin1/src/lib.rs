...
pub async fn get_jpg_tile(z: u32, x: u32, y: u32, mk: String, tk: String) -> AnyhowResult<Vec<u8>> {
    const AGENT:&str = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3";
    // 通过添加sch=wmts可返回正常XYZ顺序, 否则使用 `reversed_y: u32 = (1u32 << z) - 1 - y` 计算 -y 值
    // route 0:webp 1:jepg 2:png
    let url = format!(
        "https://api.jl1mall.com/getMap/{}/{}/{}?mk={}&tk={}&sch=wmts&vf=3&route=1",
        z, x, y, mk, tk
    );
    // 获取瓦片内容
    // 创建一个客户端并启用 Gzip 解压缩
